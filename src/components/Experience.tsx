    ...
    <section id="experience" className="relative py-24 md:py-32">
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        ...
        <div ref={timelineRef} className="mt-16 relative">
          <div className="absolute left-4 md:left-1/2 md:-translate-x-px top-0 bottom-0 w-[2px] bg-gradient-to-b from-[#C6A96B] via-[#C6A96B] to-[#C6A96B]/20" />
          <div className="relative mb-16">
            <motion.div initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="absolute left-0 md:left-1/2 md:-translate-x-1/2 z-10">
              <div className="w-8 h-8 rounded-full bg-[#C6A96B] flex items-center justify-center shadow-[0_0_20px_rgba(198,169,107,0.4)]">
                <div className="w-3 h-3 rounded-full bg-white" />
              </div>
            </motion.div>
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-100px' }} transition={{ duration: 0.7 }} className="ml-14 md:ml-0 md:w-[calc(50%-2rem)] md:mr-auto glass rounded-2xl p-6 md:p-8 hover:border-[#C6A96B]/20 transition-all duration-500">
              <div className="flex items-center gap-3 mb-4">
                <span className="px-3 py-1 rounded-full text-xs font-semibold bg-[#C6A96B]/15 text-[#C6A96B] border border-[#C6A96B]/20">2022 – Present</span>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {responsibilities.map((item, i) => (
                  <motion.div key={i} initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.06 }} className="flex items-center gap-3 text-sm text-white/60 group">
                    <item.icon size={14} className="text-[#C6A96B]/60 group-hover:text-[#C6A96B] transition-colors shrink-0" />
          ...
          {milestones.map((milestone, i) => (
            <div key={milestone.year} className="relative mb-12 last:mb-0 timeline-item">
              <div className="absolute left-0 md:left-1/2 md:-translate-x-1/2 z-10">
                <div className="w-8 h-8 rounded-full bg-[#0E0E0E] border-2 border-[#C6A96B]/40 flex items-center justify-center">
                  <div className="w-2 h-2 rounded-full bg-[#C6A96B]" />
                </div>
              </div>
              <div className={`ml-14 md:ml-0 md:w-[calc(50%-2rem)] glass rounded-2xl p-5 md:p-6 hover:border-[#C6A96B]/20 transition-all duration-500 ${i % 2 === 0 ? 'md:ml-auto' : 'md:mr-auto'}`}>
                <span className="inline-block px-3 py-1 rounded-full text-xs font-semibold bg-[#C6A96B]/10 text-[#C6A96B] border border-[#C6A96B]/20 mb-3">{milestone.year}</span>
